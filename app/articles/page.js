...
export default function Articles() {
  ...
  const [currentArticle, setCurrentArticle] = useState(null); // Track current article in viewer
  const [isViewerOpen, setIsViewerOpen] = useState(false); // Toggle viewer visibility
  const [likedArticles, setLikedArticles] = useState([]); // Track liked articles
  ...
  // Sample articles array with slugs, content, and likes
  const articles = [
  {
  ...
  {
    slug: "time-management-creatives",
  ...
  {
    slug: "automation-tools-professionals",
    category: "productivity",
  ...
  {
    slug: "effective-meeting-strategies",
    category: "productivity",
  ...
  {
    slug: "serverless-computing-trends",
    category: "technology",
    title: "Serverless Computing Trends in 2025",
    author: "Emily Carter",
    ...
    initials: "EC",
    description:
      "Learn how serverless computing is reshaping app development..."
  },
  {
    slug: "motion-design-ui",
    category: "design",
    title: "Motion Design for Engaging UI",
    ...
    initials: "LB",
    description:
      "Use motion design to create engaging and intuitive user interfaces..."
  },
  {
    slug: "agile-methodologies-2025",
    category: "business",
    title: "Agile Methodologies for 2025",
    ...
    initials: "SN",
    description:
      "Explore modern agile practices for efficient project management..."
  }
];
  ...
  // View full article with fetch
  const viewFullArticle = async (slug) => {
    try {
      // Show loading state
      setCurrentArticle({ content: "<p>Loading...</p>" });
      setIsViewerOpen(true);

      // Fetch article data (replace with your API endpoint)
      const response = await fetch(`/api/articles/${slug}`);
      if (!response.ok) throw new Error("Article not found");
      const article = await response.json();

      // Fallback: Use local articles array if no API
      ...
      setCurrentArticle({ content: "<p>Error loading article. Please try again.</p>" });
    }
  };
  ...
  // Like current article (single like per user)
  const likeCurrentArticle = async () => {
    if (!currentArticle || likedArticles.includes(currentArticle.slug)) return;

    const newLikes = currentArticle.likes + 1;
    const updatedArticle = { ...currentArticle, likes: newLikes };
    setCurrentArticle(updatedArticle);

    // Update liked articles in localStorage
    const newLikedArticles = [...likedArticles, currentArticle.slug];
    setLikedArticles(newLikedArticles);
    localStorage.setItem("likedArticles", JSON.stringify(newLikedArticles));

    // Update server (if using API)
    try {
      await fetch(`/api/articles/${currentArticle.slug}/like`, {
        method: "PATCH",
    ...
    // Update local articles array (if not using API)
    // const articleIndex = articles.findIndex((a) => a.slug === currentArticle.slug);
    // if (articleIndex !== -1) {
    //   articles[articleIndex].likes = newLikes;
    // }
  };
    ...
    if (currentArticle) {
      if (navigator.share) {
        navigator.share({
          ...
          url: `${window.location.origin}/articles/${currentArticle.slug}`,
        });
      } else {
        navigator.clipboard.writeText(`${window.location.origin}/articles/${currentArticle.slug}`).then(() => {
  ...
  return (
    <>
      ...
      <main className="container mx-auto px-6 py-16">
        ...
        <section
          id="article-viewer-section"
          className={`section-content ${isViewerOpen ? "" : "hidden"}`}
        >
          <div className="mb-6">
            <button
          ...
          <div className="bg-white rounded-3xl shadow-soft overflow-hidden">
            <div className="relative">
              <div className="absolute inset-0 sunset-gradient opacity-10"></div>
              <div className="relative p-8 border-b border-gray-100">
                <div className="flex items-start justify-between mb-6">
                    ...
                    <div className="flex items-center space-x-3 mb-4">
                      ...
                      >
                        {currentArticle?.category
                          ? currentArticle.category.charAt(0).toUpperCase() +
                            currentArticle.category.slice(1)
                          : "Category"}
                      </span>
                  ...
                  <div className="flex space-x-3">
                    <button
                      onClick={likeCurrentArticle}
                      disabled={likedArticles.includes(currentArticle?.slug)}
                      className={`flex items-center space-x-2 px-4 py-2 rounded-xl transition-colors ${
                        likedArticles.includes(currentArticle?.slug)
                    ...
                    >
                      ...
                      <span>{likedArticles.includes(currentArticle?.slug) ? "Liked" : "Like"}</span>
                    </button>
                    <button
                      onClick={shareCurrentArticle}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    >
                      ...
                      <span>{likedArticles.includes(currentArticle?.slug) ? "Liked" : "Like Article"}</span>
                    </button>
                    <button
                      onClick={shareCurrentArticle}
                  ...
                  <div className="text-sm text-gray-500">
                    Last updated: <span id="article-updated">{currentArticle?.date || "Never"}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Main Content */}
        <section id="articles-section" className={isViewerOpen ? "hidden" : ""}>
          {/* Page Header */}
              ...
              <div className="md:flex">
                ...
                <div className="md:w-1/2 p-12">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => viewFullArticle("future-web-development")}
                      className="sunset-gradient text-white px-8 py-3 rounded-full font-semibold button-hover"
                    >
                      Read Article
                    </button>
                  </div>
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            onClick={() => viewFullArticle(article.slug)}
                            className="bg-orange-100 text-orange-600 px-3 py-1 rounded-full text-xs font-semibold hover:bg-orange-200"
                          >
                            Read More
                          </button>
                        </div>
